...
alias colima-provision='colima start --cpu 10 --memory 18 --disk 50 --runtime docker --kubernetes --vm-type vz --vz-rosetta --network-address --mount ~/work/eda:w --mount ~/work/gemyn:w --mount ~/work/python-edacious-common:w && kubectl config rename-context colima docker-desktop 2>/dev/null || true'
